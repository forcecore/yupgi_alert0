...
if __name__ == "__main__" :
    # I prepared white ant using BIMP plugin for GIMP.
    # applied :
    # - to RGB
    # - brightness +100, contrast +100
    # - Then convert back to indexed color in RA1 palette.
    # I now need to copy shadow.
    # House coloring the antenna is manual work.
    # (hopefully even that shouldn't be too much with copy-paste.
    ...
    for n in range( 112 ) :
        im1 = Image.open( "ant1/ant1 {:04d}.png".format( n ) )
        im2 = Image.open( "hant/ant1 {:04d}.png".format( n ) )

        px1 = im1.load() # pixel access
        px2 = im2.load() # pixel access

        # copy shadow
        for j in range( im1.height ) :
            for i in range( im1.width ) :
                if px1[i, j] == 4 : # shadow color
                    px2[i, j] = 4

        im2.save( "hant2/hant {:04d}.png".format( n ) )
