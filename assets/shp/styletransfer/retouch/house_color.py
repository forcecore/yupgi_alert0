...
if __name__ == "__main__" :
    '''
    Transfer house colors from sno.
    '''
    house_colors = set( range( 80, 96 ) ) # house color is 80 -- 95.

    for k in range( 12 ) :
        dest = Image.open( "./des/vein {:04d}.png".format( k ) )
        src = Image.open( "./sno/vein {:04d}.png".format( k ) )

        # apply
        pd = dest.load() # pixel access
        ps = src.load() # pixel access
        for j in range( src.height ) :
            for i in range( src.width ) :
                if ps[i, j] in house_colors :
                    pd[i, j] = ps[i, j]

        dest.save( "vein {:04d}.png".format( k ) )
