...
def UTIL_WaitLoad(transport, passengers, afterLoadFunc, afterLoadParams):
    ...
    if Utils.All(passengers, lambda p: not p.IsInWorld):
...
def UTIL_BuildRandomTeam(keys):
    ...
    avail = []
    for key in keys:
        if UTIL_CanQueue(TEAMS[key]["units"]):
            avail.append(key)
